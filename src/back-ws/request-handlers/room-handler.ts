import { userRepository, type IUser } from '../db/models/users.js';
import { type IRoom, roomRepository } from '../db/models/rooms.js';
...
export const addUserToRoom = (ws: ExtendedWebSocket, data: any): void => {
  ...
  if (roomForGame.roomUsers[0].index === currentUser.id) return;

  // room already full
  if (roomForGame.roomUsers.length === 2) return;
